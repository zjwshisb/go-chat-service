...
func (c cChat) UserInfo(ctx context.Context, req *backend.ChatUserInfoReq) (res *backend.ChatUserInfoRes, err error) {
	...
	students := service.User().GetRelateStudents(user.Id)
	studentName := slice.Map(students, func(index int, item relation.Student) backend.SimpleStudent {
		return backend.SimpleStudent{
			Name:       item.Name,
			SchoolName: item.School.Name,
		}
	})
	schools := service.AdminCtx().GetSchools(ctx)
	schoolIds := slice.Map(schools, func(index int, item *entity.Schools) int {
		return item.Id
	})
	return &backend.ChatUserInfoRes{
		OrderCount: service.MealOrder().GetActiveCount(ctx, user.Id,
			do.MealOrders{SchoolId: schoolIds}),
		RefundCount: service.Refund().GetCommittedCount(ctx, user.Id),
		Phone:       user.Phone,
