...
func (m *adminManager) sendWaiting(admin *relation.CustomerAdmins, user contract.IChatUser) {
	schools := service.User().GetSchools(user.GetPrimaryKey())
	tags := ""
...
func (m *adminManager) sendOffline(admin *relation.CustomerAdmins, msg *relation.CustomerChatMessages) {
	schools := service.User().GetSchools(msg.UserId)
	tags := ""
	for _, s := range schools {
		if tags != "" {
			tags = tags + "," + s.Name
...
func (m *adminManager) unregisterHook(conn iWsConn) {
	...
	if ok {
		e := a.Entity
		e.Setting.LastOnline = time.Now()
		dao.CustomerAdminChatSettings.Ctx(gctx.New()).Save(e.Setting)
	}
	m.broadcastOnlineAdmins(conn.GetCustomerId())
}

func (m *adminManager) broadcastWaitingUser(customerId int) {
	m.broadcastLocalWaitingUser(customerId)
}

func (m *adminManager) broadcastLocalWaitingUser(customerId int) {
	sessions := service.ChatSession().GetUnAcceptModel(customerId)
	sessionIds := slice.Map(sessions, func(index int, item *relation.CustomerChatSessions) uint64 {
		return item.Id
	})
	userMap := make(map[int]*chat.WaitingUser)
...
func (m *adminManager) broadcastOnlineAdmins(gid int) {
	m.broadcastLocalOnlineAdmins(gid)
}

func (m *adminManager) broadcastLocalOnlineAdmins(customerId int) {
	admins := service.Admin().GetChatAll(customerId)
	data := make([]chat.CustomerAdmin, 0, len(admins))
	for _, c := range admins {
		conn, online := m.GetConn(customerId, gconv.Int(c.Id))
		platform := ""
		if online {
		...
		data = append(data, chat.CustomerAdmin{
			...
			Online:        online,
			Id:            c.Id,
			AcceptedCount: service.ChatRelation().GetActiveCount(gconv.Int(c.Id)),
			Platform:      platform,
		})
	}
...
func (m *adminManager) noticeLocalUserOffline(uid int) {
	adminId := service.ChatRelation().GetUserValidAdmin(uid)
	admin := service.Admin().First(adminId)
	if admin != nil {
		conn, exist := m.GetConn(admin.CustomerId, gconv.Int(admin.Id))
		if exist {
...
func (m *adminManager) noticeLocalUserOnline(uid int, platform string) {
	adminId := service.ChatRelation().GetUserValidAdmin(uid)
	admin := service.Admin().First(adminId)
	if admin != nil {
		conn, exist := m.GetConn(admin.CustomerId, gconv.Int(admin.Id))
...
func (m *adminManager) noticeUserTransfer(customerId, adminId int) {
	m.noticeLocalUserTransfer(customerId, adminId)
}

func (m *adminManager) noticeLocalUserTransfer(customerId, adminId int) {
...
func (m *adminManager) noticeUpdateSetting(customerId int, setting *entity.CustomerAdminChatSettings) {
	m.updateSetting(customerId, setting)

}

// UpdateSetting 更新设置
func (m *adminManager) updateSetting(customerId int, setting *entity.CustomerAdminChatSettings) {
	conn, exist := m.GetConn(customerId, gconv.Int(setting.AdminId))
	if exist {
