...
func (c cSession) Index(ctx context.Context, req *api.SessionIndexReq) (*api.SessionIndexRes, error) {
	...
	if req.Username != "" {
		...
		uids := slice.Map(users, func(index int, item *entity.Users) int {
			return item.Id
		})
		w["user_id"] = uids
	}
...
func (c cSession) Cancel(ctx context.Context, req *api.SessionCancelReq) (*baseApi.NilRes, error) {
...
func (c cSession) Close(ctx context.Context, req *api.SessionCloseReq) (*baseApi.NilRes, error) {
	id := ghttp.RequestFromCtx(ctx).GetRouter("id")
	session := service.ChatSession().First(ctx, do.CustomerChatSessions{
		CustomerId: service.AdminCtx().GetCustomerId(ctx),
		Id:         id,
