...
func (s *sChatSession) RelationToChat(model *relation.CustomerChatSessions) chat.Session {
	...
	username := ""
	if model.User != nil {
		username = model.User.Phone
	}
	statusLabel := ""
	status := ""
	if model.CanceledAt > 0 {
		statusLabel = "已取消"
		status = consts.ChatSessionStatusCancel
	}
	if model.CanceledAt == 0 && model.AcceptedAt == 0 {
		statusLabel = "待接入"
		status = consts.ChatSessionStatusWait
	}
	if model.AcceptedAt > 0 && model.BrokeAt == 0 {
		statusLabel = "已接入"
...
func (s *sChatSession) Create(uid int, customerId int, t int) *entity.CustomerChatSessions {
...
func (s *sChatSession) GetUnAcceptModel(customerId int) (res []*relation.CustomerChatSessions) {
...
func (s *sChatSession) ActiveTransferOne(uid int, adminId int) *entity.CustomerChatSessions {
	return s.ActiveOne(uid, adminId, consts.ChatSessionTypeTransfer)
}

func (s *sChatSession) ActiveNormalOne(uid int, adminId int) *entity.CustomerChatSessions {
	return s.ActiveOne(uid, adminId, consts.ChatSessionTypeNormal)
}

func (s *sChatSession) ActiveOne(uid int, adminId, t any) *entity.CustomerChatSessions {
