...
func (s *sChatManual) GetCountByTime(customerId uint, min string, max string) int64 {
	ctx := context.Background()
...
// GetAll 获取所有待人工接入ids
func (s *sChatManual) GetAll(customerId uint) []int64 {
	ctx := context.Background()
	val, _ := g.Redis().Do(ctx, "ZRangeByScore", s.getManualKey(customerId), &redis.ZRangeBy{
		Min:    "-inf",
		Max:    "+info",
...
func (s *sChatManual) GetBySource(customerId uint, Offset, count uint) []uint64 {
	ctx := context.Background()
