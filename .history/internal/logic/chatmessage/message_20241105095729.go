...
func (s *sChatMessage) SaveRelationOne(msg *relation.CustomerChatMessages) int64 {
...
func (s *sChatMessage) SaveOne(msg *entity.CustomerChatMessages) int64 {
	result, err := dao.CustomerChatMessages.Ctx(gctx.New()).Save(msg)
	id, err := result.LastInsertId()
	if err != nil {
		return 0
...
func (s *sChatMessage) ChangeToRead(msgId []int64) (sql.Result, error) {
	ctx := gctx.New()
